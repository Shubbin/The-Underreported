...
// Deterministic PRNG so the archive is stable across renders/SSR.
...
// Title templates per category — phrased like real wire copy, no clickbait.
...
function makeBody(title: string, state: NigerianState | undefined): string[] {
...
function fillTitle(tpl: string, state: NigerianState): string {
  return tpl
    .replace("{state}", state)
...
export const articlesByState = (s: NigerianState) => ARTICLES.filter((a) => a.state === s);
export const articlesByAuthor = (slug: string) => ARTICLES.filter((a) => a.authorSlug === slug);
export const articlesByYear = (y: number) =>
  ARTICLES.filter((a) => new Date(a.publishedAt).getFullYear() === y);

export const relatedArticles = (a: Article, n = 3) =>
  ARTICLES.filter(
    (x) => x.slug !== a.slug && (x.category === a.category || x.state === a.state),
  ).slice(0, n);

export const adjacentArticles = (a: Article) => {
